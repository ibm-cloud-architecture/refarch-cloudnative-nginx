...
RUN sed -e "s/\${api.eu.apiconnect.ibmcloud.com}/$APIC_EU_FQHN/" -e "s/\${api.us.apiconnect.ibmcloud.com}/$APIC_US_FQHN/" /tmp/load-balancer.conf > /etc/nginx/conf.d/load-balancer.conf
